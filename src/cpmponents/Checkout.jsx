...
//       <button className='btn btn-secondary byn-lg' style={{fontWeight:"bold"}} onClick={handlePayment}>Procced To Pay</button> 
...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    if (cart?.items) {
      for (let i = 0; i < cart.items?.length; i++) {
        qty += cart.items[i].qty;
        price += cart.items[i].price * cart.items[i].qty; // Quantity ke hisaab se price calculate kiya
      }
  ...
  const handlePayment = async () => {
    try {
      ...
      var options = {
        key: "rzp_test_lmCEeJVK4OMW1D", // Enter the Key ID generated from the Dashboard
      ...
      console.log(error.response || error.message); // Error message ko thoda aur detail se handle kiya
    }
  };
